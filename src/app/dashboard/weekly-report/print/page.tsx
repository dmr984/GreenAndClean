...
const WeeklyPrintPageContent = () => {
    ...
    useEffect(() => {
        ...
        if (start) {
            ...
            if (isValid(parsed)) setStartDate(startOfWeek(parsed, { weekStartsOn: 1 }));
        }
    }, [searchParams]);
...
export default function WeeklyPrintPage() {
    return <Suspense fallback={<div className="flex h-screen w-full items-center justify-center"><Loader2 className="h-12 w-12 animate-spin text-primary" /></div>}><WeeklyPrintPageContent /></Suspense>;
}